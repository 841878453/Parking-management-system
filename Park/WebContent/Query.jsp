...
<%@page import="BaseBean.*" import="java.util.ArrayList" import="java.text.DecimalFormat"%>
<%
...
<%
	ArrayList<CarBean> car_All_Infor = (ArrayList<CarBean>) session.getAttribute("car_Query_Type");
	ArrayList<String> car_Query_Record = (ArrayList<String>) session.getAttribute("car_Query_Record");
	...
	String car_Type = "";
	String car_Cost = "";
	if (car_Query_Record != null) {
		car_Approach_Time = car_Query_Record.get(0);
		car_Leaving_Time = car_Query_Record.get(1);
		car_Number = car_Query_Record.get(2);
		car_Place = car_Query_Record.get(3);
		car_Type = car_Query_Record.get(4);
		car_Cost = car_Query_Record.get(5);
	}
%>
<body>
	...
	<div class="wrapper jsc-sidebar-content jsc-sidebar-pulled"
		...
		<section class="Query_Container">
			<div class="login">
				<h1>历史信息查询</h1>
				<form method="post" action="Select_type">
					<p>
						<input type="text" name="car_Approach_Time" class="date"
							placeholder="起始时间" value="<%=car_Approach_Time%>">
					</p>
					<p>
						<input type="text" name="car_Leaving_Time" class="date"
							placeholder="结束时间" value="<%=car_Leaving_Time%>">
					</p>
					<p>
					...
					<p>
						<select name="car_Cost" class="select1">
							<option value="">停车费用</option>
							<option value="10元以下">10元以下</option>
							<option value="10-20元">10-20元</option>
							<option value="20-30元">20-30元</option>
							<option value="30-40元">30-40元</option>
							<option value="40元以上">40元以上</option>
						</select>
					</p>
		...
		<div class="Query_Table">
			<table id="table_id" class="display">
				...
				<tbody>
					<%
						if (car_All_Infor != null) {
							for (int i = 0; i < car_All_Infor.size(); i++) {
								out.print("<tr>");
								out.print("<td>" + car_All_Infor.get(i).getCar_Type() + "</td>" + "<td>"
										+ car_All_Infor.get(i).getApproach_Time().toString().substring(0, 19) + "</td>" + "<td>"
										+ car_All_Infor.get(i).getLeaving_Time().toString().substring(0, 19) + "</td>" + "<td>"
										+ decimalFormat.format((car_All_Infor.get(i).getLeaving_Time().getTime()
												- car_All_Infor.get(i).getApproach_Time().getTime()) / (1000 * 60*60.0))
										+ "h" + "</td>" + "<td>" + car_All_Infor.get(i).getCar_Number() + "</td>" + "<td>"
										+ car_All_Infor.get(i).getCar_Place() + "</td>" + "<td>"
										+ car_All_Infor.get(i).getCar_Cost() + "元</td>");
								out.print("</tr>");
							}
						}
						session.removeAttribute("car_Query_Type");
					%>
				</tbody>
			</table>
		</div>
	...
	<div class="sidebar jsc-sidebar" id="jsi-nav" data-sidebar-options="">
		<ul class="sidebar-list">
			...
			<li><a href="Query.jsp" class="current" target="_self">历史信息查询</a></li>
	...
	<%
		session.removeAttribute("car_Query_Record");
	%>
</body>
</html>